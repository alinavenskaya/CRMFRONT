...
class SettingsForm extends React.Component {
  ...
  componentWillMount() {
    if (this.props.currentUser) {
      this.setState({
        image: this.props.currentUser.userphoto || "",
        username: this.props.currentUser.userfullname,
        bio: this.props.currentUser.usertype,
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.currentUser) {
      this.setState({
        image: nextProps.currentUser.userphoto || "",
        username: nextProps.currentUser.userfullname,
        bio: nextProps.currentUser.usertype,
...
class Goals extends React.Component {
  constructor(props) {
    ...
    this.submitForm = async ev => {
      ...
      this.setState({ goal: await agent.Goals.update(goal) });
  ...
  async componentWillMount() {
    const { goal } = await agent.Goals.all();
    this.setState({ goal });
  }
  render() {
    const { classes } = this.props;
...
const mapDispatchToProps = dispatch => ({
  ...
  onSubmitGoal: goal =>
    dispatch({ type: GOAL_CREATED, payload: agent.Goals.update(goal) })
});
