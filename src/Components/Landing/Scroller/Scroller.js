...
import { Controller, Globals } from "react-spring";
...
export default class Scroller extends Component {
  ...
  handleTouchMove = e => {
    ...
    let distanceFromTouchStart =
      e.changedTouches[0].clientY - touches[0].clientY;
  ...
  render() {
    const { children, ScrollerNavigation } = this.props;

    const scroll = this.connection;

    return (
      <ScrollerContainer>
        <ScrollerNavigation scroll={scroll} />
        <ScrollerContent
...
class ScrollerContent extends PureComponent {
  render() {
    const { scroll, scrollRef, autoFrame, autoScroll, ...props } = this.props;
    ...
    return <View ref={scrollRef} style={style} {...props} />;
  }
}
